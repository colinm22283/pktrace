...
bool ConsoleParser::parse(std::string code)
{
    try
    {
                ...
                if (HELP_TEXT[i] == '\n')
                {
                    if (page <= 0)
                    {
                        ...
                        while (HELP_TEXT[j] != EOF)
                        {
                            ...
                            if (lines >= 10)
                            {
                                Console::print(std::string(HELP_TEXT).substr(firstIndex, j));
                                return true;
                            }
                            j++;
                        }
        ...
        else if (first == "set")
        {
            ...
            else if (Script::setVar && !Script::setVar(second, nstr2)) Console::print("Variable \"" + second + "\" does not exist.");
        }
        else if (first == "bind")
        {
            std::string second = nstr.substr(0, nstr.find(' '));
        ...
        {
            Console::print(Engine::getTimeAsString().c_str());
        }
        else if (Script::command && !Script::command(first, nstr))
        {
